	...
	public static boolean runYearConsumption() throws Exception {
		...
		if (confs.containsKey("reports.yearConsumption.outputTable.tableName")) {
			outputTableName = confs
					.get("reports.yearConsumption.outputTable.tableName");
		} else {
			Logger.error("ConfigError: reports.yearConsumption.outputTable.tableName doesn't exist");
			return false;
		}
		...
		if (confs.containsKey("reports.yearConsumption.dateRange.untilDate")) {
			jobArgs[1] = confs
					.get("reports.yearConsumption.dateRange.untilDate");
		} else {
			jobArgs[1] = "";
		}

		if (confs
				.containsKey("reports.yearConsumption.dataRange.onlyInTimeRange")) {
			jobArgs[2] = confs
					.get("reports.yearConsumption.dataRange.onlyInTimeRange");
		} else {
			jobArgs[2] = "false";
		}
		...
		Accumulo.getInstance().addMutation(outputTableName,
				Helpers.toByteArray("YearConsumption"),
